...
{
    // 重传RTO计算
    // 名词解析:  SRTT -smoothed round trip time
    //            RTTVAR round trip time var
    //            G TCP时钟粒度
    // 1. 经典方法(classic method)
    // SRTT = (ALPHA * SRTT) + (1 - ALPHA) * RTT 
    // RTO = min(UBOUND, max(LBOUND, BETA * SRTT))
    // 其中，ALPHA =0.8-0.9  BTA = 1.3-2.0
    //  加权平均，只能处理平滑性网络，无法适应RTT变化大的网络
    // 2. 标准方法(standard method)
    // 2.1 初始化rto
    //      rto = 1
    // 2.2 第一个rtt
    //      SRTT = rtt
